...
1. `mkdir myplugins`
1. `cp target/kafka-connect-mq-sink-*-jar-with-dependencies.jar myplugins`
1. `oc start-build <kafkaconnectClusterName>-connect --from-dir ./myplugins` to add the MQ sink connector to the Kafka Connect distributed worker cluster. Wait for the build to complete, which usually takes a few minutes.
